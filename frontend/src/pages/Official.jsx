  ...
  const mintNFT = async (land) => {
    if (land.isVerified) {
      ...
      const landData = {
        ...
        owners: land.owners,
        shares: land.shares.map(share => ethers.BigNumber.from(share)),
        price: ethers.BigNumber.from(land.price)

      }
      try {
        const tx = await nftContract.mintNft(land.owners[0], landData, { gasLimit: 8000000 });
        await tx.wait();

        console.log("your token id:", tx.value.toString());
      } catch (error) {
                  ...
                  <div className="space-y-1">
                    {selectedLand.ownerNames.map((name, index) => (
                      <div key={index} className="flex justify-between text-sm">
                        ...
                        <span>{selectedLand.shares[index].toString()}%</span>
                      </div>
                    ))}
                  </div>
                </div>
